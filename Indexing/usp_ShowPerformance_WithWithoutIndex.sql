...
BEGIN
    ...
    ----------------------------------------------------------------------
    -- Drop the practice table if it exists (reset for a clean test run)
    DROP TABLE IF EXISTS dbo.SalesOrderDetail_Practice;

    -- Copy a subset of rows into the practice table for repeatable testing
    SELECT TOP (@Rows)
           SalesOrderID,
           SalesOrderDetailID,
           ProductID,
    ...
    -- Ensure index does not exist
        -- Check for existing index so the "without index" test is accurate
        IF EXISTS (
        SELECT 1
        FROM sys.indexes
        ...
        -- Drop the index to force a scan/heap access for the baseline test
    ...
    BEGIN
        ...
        -- Baseline query: filter by ProductID without any index
    ...
    ----------------------------------------------------------------------
    -- Create a clustered index on ProductID to optimize the filter
    ...
    WHILE @i <= @Loops
    BEGIN
        -- Same query again, now expected to use the clustered index
        SELECT SalesOrderID, SalesOrderDetailID, ProductID, OrderQty, UnitPrice
        FROM dbo.SalesOrderDetail_Practice
        WHERE ProductID = @TestProductId
        OPTION (RECOMPILE);
    ...
    ----------------------------------------------------------------------
    -- Final summary of total and average timings with/without index
    SELECT
